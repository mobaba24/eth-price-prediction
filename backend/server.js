...
app.use(cors()); // In a real production setup with Nginx, this might not be needed.
app.use(express.json({ limit: '1mb' })); // Use express's body parser

// --- Gemini AI Client Initialization ---
const apiKey = process.env.GEMINI_API_KEY;
if (!apiKey) {
  console.error("GEMINI_API_KEY environment variable not set. The application will exit.");
  process.exit(1);
}
...
app.post('/api/predict', async (req, res) => {
  try {
    ...
    // No need to parse here, the Gemini API client with schema should return valid JSON text
    res.setHeader('Content-Type', 'application/json');
    res.send(jsonText);

  } catch (error) {
    console.error('Error in /api/predict endpoint:', error);
    res.status(500).json({ message: 'Failed to get prediction from AI.', details: error.message });
  }
});
